...
输入校验：productInputSchema 校验必填字段与 tone 枚举。

文案生成：generateCopyAction 调用 LLM 返回 CopyResult。

视觉策略：generateVisualStrategyAction 依据文案生成 Seedream 中文提示词 + 设计计划。

生图：generateSeedreamImageAction 调 Seedream 2304×3072，返回 URL；通过 toProxyImageUrl 包装本地代理避免 CORS。

排版：generateLayoutConfigAction 依据文案、设计计划及背景图生成 LayoutConfig。

前端编辑：Workspace 组件渲染画布，文本图层可拖拽、编辑及调整 zIndex；html-to-image 导出 1080×1440。
...
LLM ：优点是 JSON 遵从度高；缺点是排版结构有时遗漏字段、生成速度慢。
